...
# cross-model-review v1.1 ・ 跨模型互審閘門
...
| 4 | deterministic 複核（D1–D12：CONCEDE 計數防代筆、校準狀態、tier＋簽核、證據綁版本） | `scripts/grep_dangers.sh`（grep/test/sha256） | 阻擋 |
...
probe 同時回報 **POSTURE**（裁判校準姿態）：`GREEN`＝校準有效，照常。`RED`（從未校準／未通過／逾 30 天／codex 換版）＝**fail-closed：本次所有工件視同 🔴——審查照跑，但 APPROVED 後仍需使用者簽核**，並提示跑 `calibrate.sh` 重建基線。
...
- 跑 `bash $SDIR/scripts/calibrate.sh`（每題 1 次 codex 呼叫）→ 寫 `calibration.json`（日期、codex 版本、逐題結果）。通過規則：**雷題全中＋乾淨題 ≥1 過**。
- 效期 30 天；codex 版本一變即失效。過期／失效／未過 → POSTURE=RED（fail-closed 如上）。
...
**subagent 版**（codex 不可用、🟡 限定）：Agent 工具 `general-purpose`、model=`opus`（或 fable）、新 context，prompt＝上面同一份證偽 prompt（含 Edit Budget 與 5 輪上限句）＋檔案路徑。**把 subagent 回覆原文存 `$DIR/r1.txt`**（Write 工具），記下 agent 名稱／id 供 rounds 2+ 用 SendMessage 續談（對應 codex 的 resume）。reviewer 記為 `subagent:<model>`。
...
{"file":"$REVIEW_PATH","reviewer":"<審查者id>","rounds":<N>,"verdict":"approved","tier":"<red|yellow>","sha":"$SHA","started":"$(cat "$DIR/started")","finished":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
META
...
2. 呈給使用者**指名裁決**（R3：單獨「好／OK」不算）。逐項採 ACCEPT-CLAUDE／ACCEPT-REVIEWER／其他指示；照裁決改檔、ledger 改 `ARBITRATED-*`。
...
- `scripts/grep_dangers.sh` — Gate 4（D1-D10，累積式，事故後追加）
...
4. **subagent 備援只有 fresh context，沒有跨模型去相關**——🔴 工件因此強制真 codex。
...
8. **校準是「姿態級」不是「逐審級」**——calibration.json 證明的是裁判「最近、在這個版本上」可信，不是本次判決正確。hook 本身不讀校準（保持簡單、fail-open），但 fail-closed 是**機械的**：校準無效或 🔴 而無 signoff.txt ⇒ D10/D11 FAIL ⇒ gate4 無法 FAIL=0 ⇒ 證據不齊 ⇒ hook 不放行——強制經由既有證據鏈流動。
