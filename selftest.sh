...
set -u
SKILLDIR="${CROSS_REVIEW_SKILL_DIR:-$HOME/.claude/skills/cross-model-review}"   # plugin 安裝者可覆蓋
GATE="$SKILLDIR/scripts/review-gate.py"
DANGERS="$SKILLDIR/scripts/grep_dangers.sh"
...
mkcal(){ # $1=date-iso $2=overall $3=codex_version [$4=model] [$5=effort] → 寫 $T/state/calibration.json
python3 - "$T/state/calibration.json" "$1" "$2" "$3" "${4:-test-judge}" "${5:-test-effort}" <<'PY'
import json, sys
...
L
  printf '## Verdict\nAPPROVED\n' > "$d/r1.txt"
  printf '{"file":"%s","reviewer":"codex:test-judge","rounds":1,"verdict":"approved","tier":"yellow","sha":"%s"}\n' "$1" "$sha" > "$d/meta.json"
...
printf '\n<!-- cross-model-reviewed: 2026-07-05T00:00:00Z rounds=1 verdict=approved reviewer=codex:test-judge sha=%s -->\n' "$SHA" >> "$T/plans/test-plan.md"
...
printf '\n<!-- cross-model-reviewed: 2026-07-08T00:00:00Z rounds=1 verdict=approved reviewer=codex:test-judge sha=%s -->\n' "$SHA" >> "$T/plans/attack-plan.md"
...
printf '<!-- cross-model-reviewed: 2026-07-08T01:00:00Z rounds=1 verdict=approved reviewer=codex:test-judge sha=%s -->\n' "$SHA2" >> "$T/plans/attack-plan.md"
OUT=$(run_hook false)
...
printf '{"file":"%s","reviewer":"codex:test-judge","rounds":1,"verdict":"approved","tier":"yellow","sha":"%s","started":"t0","finished":"t1"}\n' "$T/plans/good-plan.md" "$GS" > "$RD/meta.json"
...
mkcal "$NOW" "PASS" "$CV" "another-judge-model"
python3 "$GATE" --calibration-check >/dev/null 2>&1 && bad "M5c 換模型應 invalid" || ok "M5c 裁判模型已變 → invalid（換模型＝換裁判）"
mkcal "$NOW" "PASS" "$CV"
python3 - "$T/state/calibration.json" <<'PY'
import json, sys
c = json.load(open(sys.argv[1])); c.pop("model", None)
json.dump(c, open(sys.argv[1], "w"))
PY
python3 "$GATE" --calibration-check >/dev/null 2>&1 && bad "M5d 舊格式應 invalid" || ok "M5d 紀錄缺 model 欄（舊格式）→ invalid"
mkcal "$NOW" "PASS" "$CV"
EMPTY_CODEX_HOME=$(mktemp -d)
( CODEX_HOME="$EMPTY_CODEX_HOME" python3 "$GATE" --calibration-check >/dev/null 2>&1 ) && bad "M5e 現行模型不明應 invalid" || ok "M5e 現行 config 無 model → invalid（fail-closed）"
rm -rf "$EMPTY_CODEX_HOME"
( CODEX_HOME="$T/codexhome" python3 "$GATE" --calibration-check >/dev/null 2>&1 ) && ok "M5g0 頂層解析不吃 [table] 內 decoy model" || bad "M5g0 被非頂層 model 干擾: $(CODEX_HOME=$T/codexhome python3 $GATE --calibration-check 2>&1)"
mkcal "$NOW" "PASS" "$CV" "test-judge" "other-effort"
python3 "$GATE" --calibration-check >/dev/null 2>&1 && bad "M5g effort 變更應 invalid" || ok "M5g 推理強度已變 → invalid（effort 是裁判身分）"
mkcal "$NOW" "PASS" "$CV"
OTHER_HOME=$(mktemp -d); printf 'model = "model-B"\nmodel_reasoning_effort = "test-effort"\n' > "$OTHER_HOME/config.toml"
( CROSS_REVIEW_JUDGE_MODEL="test-judge" CODEX_HOME="$OTHER_HOME" python3 "$GATE" --calibration-check >/dev/null 2>&1 ) && bad "M5f 殘留 env 遮蔽了換模型" || ok "M5f 紀錄=A、env=A、config=B → invalid（env 無後門）"
rm -rf "$OTHER_HOME"
...
G=$(bash "$DANGERS" "$T/plans/arb-plan.md" "$RA" 2>&1)
printf '%s' "$G" | grep -q -- "－ FAIL \[D8\].*ARBITRATED" && ok "M12b 有報告但無裁決列 → 仍擋" || bad "M12b 缺裁決列被放行"
printf '| 5 | 9 | 僵持題 | PUSHBACK | 人裁 | ARBITRATED-CLAUDE |\n' >> "$RA/ledger.md"
G=$(bash "$DANGERS" "$T/plans/arb-plan.md" "$RA" 2>&1)
printf '%s' "$G" | grep -q -- "－ FAIL \[D8\].*signoff" && ok "M12c 有裁決列但無簽核 → 仍擋" || bad "M12c 缺簽核被放行"
printf '使用者裁決：ACCEPT-CLAUDE（測試）2026-07-09\n' > "$RA/signoff.txt"
G=$(bash "$DANGERS" "$T/plans/arb-plan.md" "$RA" 2>&1)
printf '%s' "$G" | grep -q -- "＋ PASS \[D8\] 仲裁三要件" && ok "M12d 三要件齊 → D8 過" || bad "M12d 誤擋"

python3 - "$T/state/calibration.json" <<'PY2'
import json, sys
c = json.load(open(sys.argv[1])); c.pop("codex_version", None)
json.dump(c, open(sys.argv[1], "w"))
PY2
python3 "$GATE" --calibration-check >/dev/null 2>&1 && bad "M5h 紀錄缺 CLI 版本應 invalid" || ok "M5h 紀錄缺 codex_version → invalid"
mkcal "$NOW" "PASS" "$CV"
( PATH="/usr/bin:/bin" python3 "$GATE" --calibration-check >/dev/null 2>&1 ) && bad "M5i 現行版本不可得應 invalid" || ok "M5i codex 不可得 → invalid（fail-closed）"

echo "=== M15-17. 審查者身分（v1.1.1 S 系列）==="
rm -f "$RD/signoff.txt"   # 清掉 M11 殘留簽核，讓 D10 走 FAIL 路徑
python3 - "$RD/meta.json" <<'PY2'
import json, sys
m = json.load(open(sys.argv[1])); m.pop("reviewer", None)
json.dump(m, open(sys.argv[1], "w"))
PY2
G=$(bash "$DANGERS" "$T/plans/good-plan.md" "$RD" 2>&1)
printf '%s' "$G" | grep -q -- "－ FAIL \[D10\].*缺 reviewer" && ok "M15 meta 缺 reviewer → 校準不適用（不 fail-open）" || bad "M15 缺 reviewer 被放行"
python3 - "$RD/meta.json" <<'PY2'
import json, sys
m = json.load(open(sys.argv[1])); m["reviewer"] = "codex-subagent"
json.dump(m, open(sys.argv[1], "w"))
PY2
G=$(bash "$DANGERS" "$T/plans/good-plan.md" "$RD" 2>&1)
printf '%s' "$G" | grep -q -- "－ FAIL \[D10\]" && ok "M16 reviewer=codex-subagent（無冒號）→ 不得沿用" || bad "M16 矇混過關"
python3 - "$RD/meta.json" <<'PY2'
import json, sys
m = json.load(open(sys.argv[1])); m["reviewer"] = "codex:another-model"
json.dump(m, open(sys.argv[1], "w"))
PY2
G=$(bash "$DANGERS" "$T/plans/good-plan.md" "$RD" 2>&1)
printf '%s' "$G" | grep -qE -- "－ FAIL \[D10\].*只背書被校準|－ FAIL \[D13\]" && ok "M17 reviewer 與校準模型/marker 不符 → 擋" || bad "M17 漏"
python3 - "$RD/meta.json" <<'PY2'
import json, sys
...
echo "=== M14. 備援 subagent 冒用 codex 校準（R5 發現）==="
mkcal "$NOW" "PASS" "$CV"
python3 - "$RD/meta.json" <<'PY2'
import json, sys
m = json.load(open(sys.argv[1])); m["reviewer"] = "subagent:opus"
json.dump(m, open(sys.argv[1], "w"))
PY2
rm -f "$RD/signoff.txt"
G=$(bash "$DANGERS" "$T/plans/good-plan.md" "$RD" 2>&1)
printf '%s' "$G" | grep -q -- "－ FAIL \[D10\].*不適用" && ok "M14 subagent＋有效 codex 校準＋無簽核 → FAIL（不得冒用）" || bad "M14 冒用未被擋"
printf '使用者簽核：同意（測試）\n' > "$RD/signoff.txt"
G=$(bash "$DANGERS" "$T/plans/good-plan.md" "$RD" 2>&1)
printf '%s' "$G" | grep -q -- "？ WARN \[D10\]" && ok "M14b subagent＋簽核 → WARN 放行留痕" || bad "M14b 簽核未放行"
python3 - "$RD/meta.json" <<'PY2'
import json, sys
m = json.load(open(sys.argv[1])); m["reviewer"] = "codex:test-judge"
json.dump(m, open(sys.argv[1], "w"))
